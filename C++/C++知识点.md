...
`SIGSEGV` 是 **Segmentation Violation**（段违例）的缩写，是 Unix/Linux 系统中的一种信号（signal），编号通常为 11。当程序试图访问**未映射的内存地址**或**没有权限访问的内存区域**时，操作系统会向进程发送该信号，默认行为是终止进程并生成 core dump（如果开启了）。

### 常见触发场景

1. **空指针解引用**：`int *p = NULL; *p = 42;`
2. **访问已释放的内存**（悬空指针）：`free(p); *p = 42;`
3. **栈溢出**：过深的递归或过大的局部数组导致栈越界。
4. **写入只读内存**：如修改字符串字面量：`char *s = "hello"; s[0] = 'H';`
5. **数组越界**：访问数组边界之外的内存，可能碰巧未触发段错误，但也可能越界到未映射区域。
6. **使用未初始化的指针**：指针变量未赋值就解引用。

### 如何调试

- 使用 `gdb` 运行程序，段错误时会自动暂停，通过 `backtrace` 查看调用栈。
- 使用 `valgrind` 检测非法内存访问。
- 在代码中添加防御性检查（如判空）。

### 信号处理

可以捕获 `SIGSEGV` 并尝试恢复（如打印日志后安全退出），但通常不应忽略，因为程序已处于不可靠状态。捕获方法：
...
- `SIGABRT`：由 `abort()` 调用触发，通常表示程序自身检测到错误。
- `SIGBUS`：总线错误，通常指未对齐的访问或硬件故障。

### 总结

`SIGSEGV` 是内存访问越界或非法访问的信号，是 C/C++ 程序中常见的崩溃原因，通常表明程序存在严重 bug。
